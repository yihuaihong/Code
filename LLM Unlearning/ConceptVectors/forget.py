...
def evaluate(model, tokenizer, concept, location, QA, text_completion, unrelated_QA, top_k=200):
    ...
    print('QA[:6]: ', QA[:6])
    print('unrelated_QA[:6]: ', unrelated_QA[:6])
    print('qa_answers[:6]: ',qa_answers[:6])
    print('unrelated_qa_answers[:6]: ', unrelated_qa_answers[:6])
...
def main(cfg):
    ...
    random_wikipedia_content = [running_set[1]['wikipedia_content']]
    ...
    # #early evaluate for debug
    # evaluate(model, tokenizer=tokenizer, concept=concept, location=location, QA=QA, text_completion=Text_completion,
    ...
    if not os.path.exists(cfg.results_save_path):
        os.makedirs(cfg.results_save_path)
    ...
    # Optional
    model.save_pretrained(cfg.results_save_path)
    tokenizer.save_pretrained(cfg.results_save_path)
