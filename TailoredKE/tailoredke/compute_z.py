...
def compute_z(
    ...
    true_target_ids = tok(request["target_true"]["str"], return_tensors="pt").to("cuda")[
        "input_ids"
    ][0]
    ...
    if dynamic_layers == True:
        # TailoredKE Dynamic Layer Selection
        layer_to_edit = fine_edit_layer(
            model,
            tok,
            input_tok,
            lookup_idxs,
            true_target_ids=true_target_ids,
            request=request,
            hparams=hparams,
        )
